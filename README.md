...
A local Retrieval-Augmented Generation (RAG) system designed to answer questions about corporate policies (e.g., travel, HR, and reimbursement). This project utilizes FastAPI for the backend API, Gradio for a real-time streaming user interface, and Ollama for local LLM inference. It features a robust Two-Stage Retrieval pipeline and an automated LLM-as-a-Judge evaluation framework to mathematically verify accuracy.

## Architecture & Tech Stack

* **Generation Model:** `phi3.5:latest` (via local Ollama) to handle complex conditional logic and strict instruction following. The system prompt utilizes advanced XML framing.
* **Retrieval Pipeline (Two-Stage)**: 
    * **Bi-Encoder (Initial Search)**: `BAAI/bge-small-en-v1.5` (via SentenceTransformers). A lightweight embedding model that generates dense 384-dimensional vectors to quickly cast a wide net and pull the top 15 candidate chunks.
    * **Cross-Encoder (Re-Ranking)**: `cross-encoder/ms-marco-MiniLM-L-6-v2`. A highly accurate reranker that processes the user's exact query and the candidate documents simultaneously, calculating a precise semantic relationship score to surface the absolute best 3 chunks.
* **Automated Evaluation (LLM-as-a-Judge)**: Integrated with the Ragas framework, utilizing `gpt-4o-mini` (via Langchain wrappers) to mathematically grade the pipeline's performance against a Golden Dataset. Current benchmark metrics:
    * **Context Precision**: 0.9500 (Exceptional document retrieval and ranking)
    * **Faithfulness**: 0.8425 (Strong adherence to retrieved facts with minimal hallucination)
    * **Answer Correctness**: 0.6137 (Semantic equivalence to ground truth)
...
│   ├── evaluation_results.csv # Contains the eval data generated by run_evaluation.py
│   └── golden_dataset.json    # Contains the ground truth data
├── src/
│   ├── __init__.py
